...
const TICK_MS = 2000; // 2s — halves DB writes vs 1s; progress interpolated from wall time
const RENDER_THROTTLE_MS = 5000;
const MIN_MINUTES_PER_HOUR = 1;
const MAX_MINUTES_PER_HOUR = 60;
const DEFAULT_MINUTES_PER_HOUR = 5;
const MS_PER_CAMPAIGN_HOUR = 60 * 60 * 1000;
...
function _clamp_minutes_per_hour(rate) {
  return Math.min(
    MAX_MINUTES_PER_HOUR,
    Math.max(MIN_MINUTES_PER_HOUR, Number(rate) || DEFAULT_MINUTES_PER_HOUR),
  );
}

/** Real-world ms to advance in_game_ms at the given rate (minutes per campaign hour). */
function _wall_duration_ms(in_game_ms, minutes_per_hour) {
  const rate = _clamp_minutes_per_hour(minutes_per_hour);
  const in_game_hours = in_game_ms / MS_PER_CAMPAIGN_HOUR;
  return in_game_hours * rate * 60 * 1000;
}

export function estimate_real_minutes(amount, unit, minutes_per_hour) {
  const in_game_ms = _in_game_ms(amount, unit);
  if (in_game_ms <= 0) return 0;
  const in_game_hours = in_game_ms / MS_PER_CAMPAIGN_HOUR;
  return Math.round(in_game_hours * _clamp_minutes_per_hour(minutes_per_hour));
}
...
function _compute_progress(state) {
  if (!state.active || !state.wall_duration_ms) {
    return { progress: 0, eta_ms: 0, current_ut: game.dc.get_unixtime() };
  }
...
export function get_auto_time_public_state() {
  ...
  return {
    ...
    minutes_per_hour: state.minutes_per_hour ?? DEFAULT_MINUTES_PER_HOUR,
    estimated_real_minutes: estimate_real_minutes(state.amount, state.unit, state.minutes_per_hour),
  };
}
...
async function _flush_full_effects(from_ut, to_ut) {
  ...
  // effects: "full" forces a DB flush + runs healing/temp_boon tick.
  // old_ut ensures set_unixtime sees the delta even if the shadow is already at to_ut.
...
export async function start_auto_time({ amount, unit, minutes_per_hour } = {}) {
  if (!game.user.isGM) return;
  ...
  const start_ut = game.dc.get_unixtime();
  const in_game_ms = _in_game_ms(amount, unit);
  if (in_game_ms <= 0) {
  ...
  const state = {
    ...
    wall_start: Date.now(),
    wall_duration_ms: _wall_duration_ms(in_game_ms, rate),
    last_flushed_ut: start_ut,
    amount: Number(amount) || 1,
    unit: unit || "hours",
    minutes_per_hour: rate,
  };
...
export async function stop_auto_time() {
  ...
  const current_ut = game.dc.get_unixtime();

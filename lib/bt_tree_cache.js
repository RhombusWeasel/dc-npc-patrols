...
import { hash_string } from "./hash_string.js";

/** @type {Map<string, { hash: number, root: object }>} */
const _cache = new Map();

function _tree_hash(tree) {
	// Cache the hash on the tree object itself. get_bt() returns a stable
	// reference from the module's cached world settings, and save/delete
	// replace it with a fresh object, so the cached hash is only stale when
	// the tree has actually been replaced — never while ticking.
	if (tree._bt_hash != null) return tree._bt_hash;
	tree._bt_hash = hash_string(JSON.stringify(tree));
	return tree._bt_hash;
}

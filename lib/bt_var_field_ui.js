...
/**
 * Build variable {{key}} option HTML for typed selects.
 * @param {object[]} var_defs
 * @param {(def: object) => boolean} [filter]
 */
export function build_variable_option_tags(var_defs, filter = null) {
  const escape = foundry.utils.escapeHTML;
  const tags = [];
  for (const def of var_defs || []) {
    ...
    tags.push(`<option value="${escape(tag)}">${escape(label)}</option>`);
  }
  return tags;
}

/**
 * Populate a direction select with optional variables + compass directions.
 * @param {HTMLSelectElement} sel
 * @param {object} opts
 */
export function populate_direction_select(sel, {
  var_defs = [],
  current_value = "",
  ...
  if (!sel) return;
  const escape = foundry.utils.escapeHTML;
  let inner = blank_label ? `<option value="">${escape(blank_label)}</option>` : "";

  if (include_variables) {
    const var_opts = build_variable_option_tags(var_defs, variable_filter);
    if (var_opts.length) {
      inner += `<optgroup label="${escape(game.i18n.localize("dc-npc-patrols.editor.var_options"))}">${var_opts.join("")}</optgroup>`;
    }
  }

 ...
 */
export function wire_attachment_events(html, selected_actor, region_manager) {
	...
	html.querySelectorAll("[data-remove-dialog]").forEach((el) => {
		el.addEventListener("click", async () => {
			...
			await _remove_attachment(selected_actor, "dialog", idx, region_manager);
		});
	});
	...
	html.querySelectorAll("[data-remove-ambient]").forEach((el) => {
		el.addEventListener("click", async () => {
			...
			await _remove_attachment(selected_actor, "ambient", idx, region_manager);
		});
	});
}
...
async function _add_attachment(actor, kind, attachment, region_manager) {
...
async function _remove_attachment(actor, kind, idx, region_manager) {
	const flag_key = kind === "dialog" ? "dialog_attachments" : "ambient_attachments";
	const list = foundry.utils.duplicate(actor.getFlag(MODULE_ID, flag_key) || []);

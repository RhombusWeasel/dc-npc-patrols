...
export function _get_region_cells(region, gw, gh, cell_size) {
	const cache_key = _region_cells_key(region, gw, gh, cell_size);
	if (cache_key) {
		const cached = _region_cells_cache.get(cache_key);
		if (cached) return cached;
	}

	const cells = _compute_region_cells(region, gw, gh, cell_size);

	if (cache_key) _region_cells_cache.set(cache_key, cells);
	return cells;
}

const _region_cells_cache = new Map();

function _region_cells_key(region, gw, gh, cell_size) {
	const scene_id = region?.parent?.id;
	const region_id = region?.id;
	if (!scene_id || !region_id) return null;
	return `${scene_id}:${region_id}:${gw}x${gh}:${cell_size}`;
}

/** Invalidate the region-cell rasterization cache for a scene (or all). */
export function invalidate_region_cells_cache(scene_id = null) {
	if (scene_id) {
		for (const key of _region_cells_cache.keys()) {
			if (key.startsWith(`${scene_id}:`)) _region_cells_cache.delete(key);
		}
	} else {
		_region_cells_cache.clear();
	}
}

function _compute_region_cells(region, gw, gh, cell_size) {
	const cells = [];
...
export function _evaluate_operator(actual, operator, expected) {
	// Prefer the system's shared condition evaluator when available.
	const ce = game.dc?.condition_eval;
	if (ce?.evaluate) return ce.evaluate(actual, operator, expected);
	// Fallback — deployed system may not expose condition_eval yet.
	switch (operator) {
...
export async function _animate_token_travel(token_doc, to_x, to_y) {
	...
	await token_doc.update(upd, { render: false });
}
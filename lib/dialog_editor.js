...
export class DialogEditorController {
	...
	async prepare_context() {
		...
		// Convert nested folders into render groups with indentation metadata.
		const folder_groups = [];
		const _walk = (folders, depth) => {
			for (const fe of folders) {
				folder_groups.push({
					is_folder: true,
					folder_id: fe.id,
					folder_name: fe.name,
					depth,
					trees: fe.trees,
					children: fe.children.length,
				});
				_walk(fe.children, depth + 1);
			}
		};
		_walk(roots, 0);

...
export class DialogEditorController {
	...
	async prepare_context() {
		...
		if (this.selected_tree_id && trees[this.selected_tree_id]) {
			...
			if (this.selected_node_id && selected_tree.nodes?.[this.selected_node_id]) {
				...
				selected_node.diverts = (selected_node.diverts || []).map(d => ({ conditions: d.conditions || [], goto: d.goto || null }));
				if (!selected_node.is_ref) {
	...
	_wire_flag_conditions(html, host) {
		...
		html.querySelectorAll("[data-divert-cond-delete]").forEach((el) => {
			el.addEventListener("click", () => {
				const [d_idx, c_idx] = el.dataset.divertCondDelete.split(":").map(Number);
				if (!this.working_tree || !this.selected_node_id) return;
				const node = this.working_tree.nodes[this.selected_node_id];
				const conds = node?.diverts?.[d_idx]?.conditions;
				if (conds) {
					conds.splice(c_idx, 1);
					this._save_working_tree();
					host.render();
				}
			});
		...
		html.querySelectorAll("[data-divert-cond-add]").forEach((el) => {
			el.addEventListener("click", () => {
				const d_idx = parseInt(el.dataset.divertCondAdd, 10);
				if (!this.working_tree || !this.selected_node_id) return;
				const node = this.working_tree.nodes[this.selected_node_id];
				const divert = node?.diverts?.[d_idx];
				if (!divert) return;
				divert.conditions = divert.conditions || [];
				divert.conditions.push({ flag_key: "", operator: "exists", expected_value: "", scope: "actor" });
				this._save_working_tree();
				host.render();
			});
		});
		...
		html.querySelector("[data-action='add-node-divert']")?.addEventListener("click", () => {
			...
			node.diverts.push({ conditions: [], goto: null });
			this._save_working_tree();
			host.render();
		});

...
export class DialogEditorController {
	...
	async prepare_context() {
		...
		for (const f of Object.values(folders)) {
			folder_groups.push({
				is_folder: true,
				folder_id: f.id,
				folder_name: f.name || "(unnamed)",
				trees: tree_entries.filter((t) => t.folder === f.id),
			});
		}
		folder_groups.push({
			is_folder: true,
			...
			trees: tree_entries.filter((t) => t.folder === ""),
		});
	...
	async _add_folder(host) {
		...
		const folder = make_folder(name.trim());

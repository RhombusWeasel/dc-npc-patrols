...
export class DialogEditorController {
		...
		html.querySelectorAll("[data-node-field]").forEach((el) => {
			el.addEventListener("change", () => {
				...
				if (field === "id") {
					...
					if (field === "tree_id" || field === "return_to") {
		...
		html.querySelectorAll("[data-response-field]").forEach((el) => {
			el.addEventListener("change", () => {
				...
				if (field === "once") {
					...
					// Stored verbatim — the GM's string is the source of truth;
					// tokens are split/expanded at write time by the runner.
					response.set_flags = el.value.trim();
	...
	_wire_flag_conditions(html, host) {
		// ── Response-level flag conditions ── (node-level routing is diverts-only)

...
/** Prefer token_doc.actor when a scene token is known. */
export function resolve_actor(actor, token_doc = null) {
...
export function resolve_gear_path(actor, label) {
	...
	if (!matches.length) {
		const ce = game.dc?.condition_eval ?? sys_condition_eval;
		const fallback = ce.gear_path_by_label(actor.system.char.gear, label);
...
export async function use_item(actor, relative_gear_path) {
	...
	if (ctx.satisfy_craving_indices?.length) {
		const now = game.dc.get_unixtime();

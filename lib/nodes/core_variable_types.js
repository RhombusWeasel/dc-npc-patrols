...
// Note: get_waypoint_options, get_region_options, get_door_options remain in bt_variables.js
...
export function register_core_variable_types() {
	...
	register_variable_type({
		...
		coerce: (raw, default_val = "") => {
			...
			if (empty) {
				...
				return default_val === true || default_val === "true" || default_val === 1 || default_val === "1";
			}
	...
	register_variable_type({
		...
		build_field: (def, raw, has_value, options = {}) => ({
			key: def.key,
			label: def.label || def.key,
			type: "waypoint_select",
			default: def.default ?? "",
			value: has_value ? raw : "",
			display_value: has_value ? raw : (def.default ?? ""),
			options: options.waypoint_options ?? [],
		}),
	});

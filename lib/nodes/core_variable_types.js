...
import { coerce_face_direction, get_face_direction_option_list } from "../face_directions.js";
...
export function register_core_variable_types() {
	...
	register_variable_type({
		id: "dialog_tree_select",
		label: "Dialog Tree Select",
		default_value: () => "",
		coerce: (raw, default_val = "") => {
			const empty = raw === undefined || raw === null || raw === "";
			if (empty) {
		...
		build_field: (def, raw, has_value, options = {}) => ({
			key: def.key,
			label: def.label || def.key,
			type: "dialog_tree_select",
			default: def.default ?? "",
			value: has_value ? raw : "",
			display_value: has_value ? raw : (def.default ?? ""),
			options: options.dialog_tree_options ?? [],
		}),
	});

	register_variable_type({
		id: "ambient_set_select",
		label: "Ambient Set Select",
		default_value: () => "",
		coerce: (raw, default_val = "") => {
			const empty = raw === undefined || raw === null || raw === "";
			if (empty) {
		...
		build_field: (def, raw, has_value, options = {}) => ({
			key: def.key,
			label: def.label || def.key,
			type: "ambient_set_select",
			default: def.default ?? "",
			value: has_value ? raw : "",
			display_value: has_value ? raw : (def.default ?? ""),
			options: options.ambient_set_options ?? [],
		}),
	});
	...
	register_variable_type({
		...
		build_field: (def, raw, has_value, _options) => ({
			...
			value: has_value ? raw : "",
			display_value: has_value ? raw : (def.default ?? ""),
			options: _options.fragment_options ?? [],
		}),
	});
	...
	register_variable_type({
		...
		default_value: () => "S",
		coerce: (raw, default_val = "S") => coerce_face_direction(raw, default_val),
		build_field: (def, raw, has_value, _options) => ({
			key: def.key,
			label: def.label || def.key,
			type: "direction_select",
			default: def.default ?? "S",
			value: has_value ? raw : "",
			display_value: has_value ? raw : (def.default ?? "S"),
			options: get_face_direction_option_list(),
		}),
	});
}
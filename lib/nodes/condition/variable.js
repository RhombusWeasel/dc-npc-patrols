...
export function register() {
	register_node("condition_variable", {
		...
		tick: async (node, bb) => {
			...
			console.warn(`[dc-npc-patrols|debug:variable] raw_key="${node.variable_key}" stripped_key="${variable_key}" actual=${JSON.stringify(actual)} (type=${typeof actual}) bb.variables=${JSON.stringify(bb.variables)}`);
			...
			console.warn(`[dc-npc-patrols|debug:variable] raw_key="${node.variable_key}" stripped_key="${variable_key}" actual=${JSON.stringify(actual)} expected=${JSON.stringify(expected)} operator="${node.operator}" result=${result}`);

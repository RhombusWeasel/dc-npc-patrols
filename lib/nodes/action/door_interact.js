...
import { resolve_wall, set_door_state, door_state_from_key, is_token_adjacent_to_door, get_actor_key_uuids } from "../../doors.js";
...
export function register() {
	register_node("action_door_interact", {
		...
		tick: async (node, bb, engine) => {
			...
			const path = await engine.pathfinding.find_path_to_wall(
				bb.scene,
				{ x: center.x, y: center.y, level_id: bb.level_id },
				wall,
				{ exclude_token_id: bb.token.id, key_uuids: get_actor_key_uuids(bb.actor) },
			);
			if (!path?.length) return Status.FAILURE;

			bb[move_key] = create_move_state(path, bb, { wall_id, target_state });

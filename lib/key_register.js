 ...
 * Checks if the player's token is adjacent to a door matching one of the
 * wall_uuids (comma-separated) and sends a socket request to the GM to unlock it.
 */
async function use_handler(actor, path, key, item) {
	...
	const wall_uuids = String(key_data.wall_uuid ?? "")
		.split(",")
		.map((s) => s.trim())
		.filter(Boolean);
	if (!wall_uuids.length) {
	...
	// Resolve each linked wall; unlock the first adjacent, locked door.
	for (const wall_uuid of wall_uuids) {
		const wall = await fromUuid(wall_uuid);
		if (!wall?.isDoor) continue;
		if (wall.parent?.id !== scene.id) continue;

...
import { _tile_has_change_level } from "./utils.js";
...
export class PatrolEngine {
	...
	async _animate_to(token_doc, wp) {
		...
		// Route intermediates are always on the same level, so a regular update is fine.
		if (wp.route && wp.route.length > 0) {
			for (const rp of wp.route) {
				await token.animate({ x: rp.x * grid, y: rp.y * grid });
		...
		// Check if the destination tile contains a Change Level region (stairs).
		// If so, use a single move() with action: "displace" so the Change Level
		// confirmation dialog never fires. The visual animation still plays via
		// token.animate() before the persist.
		const cl_region = _tile_has_change_level(token_doc.parent, px_x, px_y, grid);

		if (cl_region) {
			// Animate visually, then persist with displace (skips Change Level dialog)
			const anim_data = { x: px_x, y: px_y };
			if (wp.face_direction != null) anim_data.rotation = wp.face_direction;
			await token.animate(anim_data);

			// Determine target level from the waypoint or the region's connected levels
			const connected = cl_region.levels.size
				? [...cl_region.levels]
				: token_doc.parent.levels.contents.map(l => l.id);
			// Pick the level that isn't the current one (if available)
			const target_level = (wp.level_id && wp.level_id !== '_default')
				? wp.level_id
				: (connected.find(l => l !== token_doc.level) ?? token_doc.level);
			const level_doc = target_level !== '_default' ? token_doc.parent.levels.get(target_level) : null;

			await token_doc.move([{
				x: px_x, y: px_y,
				level: target_level,
				elevation: level_doc?.elevation?.base ?? token_doc.elevation,
				action: "displace",
			}], { animate: false });
			if (wp.face_direction != null) {
				await token_doc.update({ rotation: wp.face_direction }, { animate: false, diff: false });
			}
		} else {
			// Normal movement: animate then persist with update
			const anim_data = { x: px_x, y: px_y };
			if (wp.face_direction != null) anim_data.rotation = wp.face_direction;
			await token.animate(anim_data);

			const update_data = { x: px_x, y: px_y };
			if (wp.face_direction != null) update_data.rotation = wp.face_direction;

...
export class PatrolEngine {
	...
	async evaluate_schedules(old_unixtime, new_unixtime) {
		...
		// Get DC campaign time for old and new
		const old_date = game.dc.utils.time.get_date(new Date(old_unixtime));
		const new_date = game.dc.utils.time.get_date(new Date(new_unixtime));
		...
		const time_advanced = new_minutes > old_minutes ||
		...
		try {
			for (let i = 0; i < npc_tokens.length; i++) {
				...
				const due_waypoints = this._find_due_waypoints(active_path, old_minutes, new_minutes, time_advanced, old_date, new_date);
	...
	_find_active_path(paths, weekday) {
		// Prefer a path that explicitly includes this day
		...
		// Fall back to first enabled path
		return paths.find((p) => p.enabled) || null;
	}

	// --- Find waypoints whose arrival time falls between old and new ---
	_find_due_waypoints(path, old_minutes, new_minutes, time_advanced, old_date, new_date) {
		const due = [];
		for (const wp of path.waypoints) {
			...
			if (time_advanced && new_date.day !== old_date.day) {
		...
		due.sort((a, b) => {
			...
			const am = ah * 60 + (parseInt(a.time.split(":")[1]) || 0);
			const bm = bh * 60 + (parseInt(b.time.split(":")[1]) || 0);
			return am - bm;
		});
		return due;
	}
	...
	async _execute_movement(token_doc, actor, due_waypoints) {
		for (const wp of due_waypoints) {
			...
			// Update proximity region
			await this.region_manager.update_for_waypoint(token_doc, actor, wp);
		}
	}
	...
	async _animate_to(token_doc, wp) {
		...
		// Build route: current position → waypoint
		const route = [{ x: token_doc.x, y: token_doc.y }, { x: wp.x, y: wp.y }];

		// Use Foundry's built-in movement animation
		const speed = game.settings.get(this.module_id, "movement_speed");
		await token.animateMovement(route, { speed });

		// Update the token document position and face direction
		const update_data = { x: wp.x, y: wp.y };

...
import { _animate_token_travel, _tile_has_change_level, TOKEN_MOVE_OPTS } from "./utils.js";
...
export class PatrolEngine {
	...
	async evaluate_schedules(old_unixtime, new_unixtime) {
		...
		const combat_active = is_dc_combat_active();
	...
	async _animate_to(token_doc, wp) {
		...
		if (wp.route && wp.route.length > 0) {
			for (const rp of wp.route) {
				await _animate_token_travel(token_doc, rp.x * grid, rp.y * grid);
			}
		}
		...
		if (cl_region) {
			await _animate_token_travel(token_doc, px_x, px_y);
			...
			await token_doc.move([{
				...
				action: "displace",
			}], { ...TOKEN_MOVE_OPTS, animate: false });
		} else {
			await _animate_token_travel(token_doc, px_x, px_y);
		}

		// Optional arrival facing overrides travel direction (e.g. counter-facing NPC)
		if (wp.face_direction != null) {
			await token_doc.update({ rotation: wp.face_direction }, { animate: true, diff: false });
		}
	}

...
import {
  populate_variable_condition_fields,
  render_var_default_field_html,
  read_var_default_from_element,
  coerce_var_default_on_type_change,
  default_for_var_type,
  get_var_editor_options,
  populate_select_element,
  populate_direction_select,
  lookup_var_def,
  is_select_var_type,
} from "./bt_var_field_ui.js";
import { get_trees, get_ambient_sets } from "./dialog_tree_store.js";
import { get_builtin_fragments, get_builtin_fragment } from "./fragments/builtin_fragments.js";
...
function _get_node_summary(node) {
	...
	const fields = _get_node_fields(node);
	for (const field of fields) {
		if (field.type === "boon_list") continue;
...
function _get_field_value(node, field) {
	...
	if (field.type === "boon_list") {
		return Array.isArray(val) ? val : (field.default ?? []);
	}
...
function _get_node_type_options() {
	const categories = { composite: [], decorator: [], condition: [], action: [], utility: [], reference: [] };
...
/** @type {object[]|null} */
let _palette_nodes_cache = null;

function _get_palette_nodes() {
	if (_palette_nodes_cache) return _palette_nodes_cache;

	const order = ["composite", "decorator", "condition", "action", "utility", "reference"];
	const result = [];
...
export class BTEditorController {
	...
	async prepare_context() {
		...
		if (this.selected_bt_id && bts[this.selected_bt_id]) {
			...
			if (this.selected_node_id) {
				...
				if (found) {
					...
					if (selected_node.type === "condition_variable") {
						populate_variable_condition_fields(
							selected_node,
							node_fields,
		...
		const builtin_fragments = get_builtin_fragments().map((f) => ({
			id: f.id,
			name: f.name,
	...
	wire_events(html, host) {
		...
		html.querySelectorAll("[data-node-id]").forEach((el) => {
			el.addEventListener("click", (ev) => {
				if (ev.target.closest("[data-node-delete]")) return;
				this.selected_node_id = el.dataset.nodeId;
				host.render();
			});
		});

		html.querySelectorAll("[data-node-delete]").forEach((el) => {
			el.addEventListener("click", async (ev) => {
				ev.stopPropagation();
			const node_id = el.dataset.nodeDelete;
			this._delete_node_by_id(node_id, host);
			});
		});
		...
		html.querySelectorAll("[data-node-prop]").forEach((el) => {
			el.addEventListener("change", () => {
				...
				if (field_def?.type === "number" && el.type !== "checkbox") {
				...
				if (found.node.type === "condition_variable" && key === "expected_value") {
					const var_def = lookup_var_def(this.working_bt?.variables || [], found.node.variable_key);
				...
				if (found.node.type === "condition_variable" && key === "variable_key") {
					host.render();
					return;
				}
...
this._refresh_variables_rows(html);
		...
		this._populate_var_default_selects(html);
		this._populate_region_selects(html);
		this._populate_door_selects(html);
		this._populate_direction_selects(html);
		this._populate_fragment_selects(html);
		this._populate_dialog_tree_selects(html);
		this._populate_ambient_set_selects(html);
		this._populate_condition_variable_selects(html);
	...
	_inject_boon_editors(html, host) {
		...
		html.querySelectorAll("[data-boon-list]").forEach((container) => {
			const field_key = container.dataset.boonList;
			if (!node[field_key]) node[field_key] = [];
			const boons = node[field_key];

			container.innerHTML = BoonEditor._render_boon_list_table(boons);
			...
			container.querySelectorAll(".boon-list-copy").forEach((btn) => {
				btn.addEventListener("click", () => {
					const boon_idx = parseInt(btn.dataset.idx, 10);
					const clone = foundry.utils.deepClone(boons[boon_idx]);
	...
	_render_variable_row_html(var_def, idx) {
		...
		return `<div class="bt-var-row" data-var-index="${idx}">
			...
			${render_var_default_field_html(var_def, escape)}
	...
	_wire_variable_events(html, host) {
		...
		if (add_btn && !add_btn.dataset.wired) {
			...
			add_btn.addEventListener("click", async () => {
				...
				this.working_bt.variables.push({ key: "", label: "", type: "text", default: default_for_var_type("text") });
		...
		html.querySelectorAll("[data-var-field]").forEach((el) => {
			...
			el.addEventListener("change", async () => {
				...
				if (field === "key") {
					...
					var_def.default = coerce_var_default_on_type_change(var_def, el.value);
					await this._save_working_bt();
					this._refresh_variables_rows(html);
					this._wire_variable_events(html, host);
					this._populate_var_default_selects(html);
					...
					var_def.default = read_var_default_from_element(el, var_def.type || "text");
				} else {
	...
	_populate_var_default_selects(html) {
		if (!this.working_bt?.variables?.length) return;
		for (let i = 0; i < this.working_bt.variables.length; i++) {
			const def = this.working_bt.variables[i];
			const row = html.querySelector(`.bt-var-row[data-var-index="${i}"]`);
			const sel = row?.querySelector('select[data-var-field="default"]');
			if (!sel || !is_select_var_type(def.type)) continue;

			if (def.type === "direction_select") {
				populate_direction_select(sel, {
					var_defs: this.working_bt.variables,
			...
			populate_select_element(sel, get_var_editor_options(def.type), def.default ?? "", {
				blank_label: "(blank)",
			});
		}
	}

	_populate_condition_variable_selects(html) {
		if (!this.selected_node_id || !this.working_bt) return;
		const found = _find_node(this.working_bt.root, this.selected_node_id);
		if (found?.node?.type !== "condition_variable") return;

		const var_def = lookup_var_def(this.working_bt.variables || [], found.node.variable_key);
		if (!var_def || !is_select_var_type(var_def.type)) return;

		const sel = html.querySelector('[data-node-prop="expected_value"]');
		if (!sel || sel.tagName !== "SELECT") return;

		if (var_def.type === "direction_select") {
			populate_direction_select(sel, {
				var_defs: this.working_bt.variables,
				current_value: found.node.expected_value ?? "",
		...
		populate_select_element(sel, get_var_editor_options(var_def.type), found.node.expected_value ?? "", {
			blank_label: "(none)",
		});
	}
	...
	_populate_direction_selects(html) {
		const var_defs = this.working_bt?.variables || [];

		let current_direction = "";
		if (this.selected_node_id && this.working_bt) {
			const found = _find_node(this.working_bt.root, this.selected_node_id);
			if (found?.node?.direction) current_direction = found.node.direction;
		}

		html.querySelectorAll("select[data-node-prop='direction']").forEach((sel) => {
			populate_direction_select(sel, {
				var_defs,
	...
	_populate_dialog_tree_selects(html) {
		const escape = foundry.utils.escapeHTML;

		const variable_opts = [];
		for (const def of this.working_bt?.variables || []) {
			if (def.type !== "dialog_tree_select") continue;
			const key = def.key?.trim();
			if (!key) continue;
			const tag = `{{${key}}}`;
			const label = def.label ? `${tag} (${def.label})` : tag;
		...
		// Build dialog tree options
		const tree_opts = Object.values(get_trees()).map(
			(t) => `<option value="${escape(t.id)}">${escape(t.name || t.id)}</option>`
		);

		let current_tree_id = "";
		if (this.selected_node_id && this.working_bt) {
			const found = _find_node(this.working_bt.root, this.selected_node_id);
			if (found?.node?.tree_id) current_tree_id = found.node.tree_id;
		}

		html.querySelectorAll("select[data-node-prop='tree_id']").forEach((sel) => {
			let inner = '<option value="">(none — clear existing)</option>';
			if (variable_opts.length) {
				inner += `<optgroup label="${escape(game.i18n.localize("dc-npc-patrols.editor.var_options"))}">${variable_opts.join("")}</optgroup>`;
			}
			if (tree_opts.length) {
				inner += `<optgroup label="Dialog Trees">${tree_opts.join("")}</optgroup>`;
			}
			sel.innerHTML = inner;
			sel.value = current_tree_id;
		});
	}

	_populate_ambient_set_selects(html) {
		const escape = foundry.utils.escapeHTML;

		const variable_opts = [];
		for (const def of this.working_bt?.variables || []) {
			if (def.type !== "ambient_set_select") continue;
			const key = def.key?.trim();
			if (!key) continue;
			const tag = `{{${key}}}`;
			const label = def.label ? `${tag} (${def.label})` : tag;
		...
		// Build ambient set options
		const set_opts = Object.values(get_ambient_sets()).map(
			(s) => `<option value="${escape(s.id)}">${escape(s.name || s.id)}</option>`
		);

		let current_set_id = "";
		if (this.selected_node_id && this.working_bt) {
			const found = _find_node(this.working_bt.root, this.selected_node_id);
			if (found?.node?.set_id) current_set_id = found.node.set_id;
		}

		html.querySelectorAll("select[data-node-prop='set_id']").forEach((sel) => {
			let inner = '<option value="">(none — clear existing)</option>';
			if (variable_opts.length) {
				inner += `<optgroup label="${escape(game.i18n.localize("dc-npc-patrols.editor.var_options"))}">${variable_opts.join("")}</optgroup>`;
			}
			if (set_opts.length) {
				inner += `<optgroup label="Ambient Sets">${set_opts.join("")}</optgroup>`;
			}
			sel.innerHTML = inner;
			sel.value = current_set_id;
		});
	}
	...
	_wire_drag_drop(html, host) {
		...
		// ── Fragment chips (user + built-in): make draggable ──
		html.querySelectorAll("[data-palette-fragment]").forEach((chip) => {
			chip.setAttribute("draggable", "true");
			chip.addEventListener("dragstart", (ev) => {
				ev.dataTransfer.setData("text/plain", JSON.stringify({
					source: "palette_fragment",
		...
		html.querySelectorAll("[data-palette-builtin-fragment]").forEach((chip) => {
			chip.setAttribute("draggable", "true");
			chip.addEventListener("dragstart", (ev) => {
				ev.dataTransfer.setData("text/plain", JSON.stringify({
					source: "palette_builtin_fragment",
					fragment_id: chip.dataset.paletteBuiltinFragment,
				}));
				ev.dataTransfer.effectAllowed = "copy";
				chip.classList.add("bt-palette-chip-dragging");
			});
		...
		// ── Empty area: drop = add as last child of root (fallback for all drops) ──
		// Attached to the structure panel body (fills entire center column) so
		// drops anywhere in the center area are caught, not just on tree nodes.
		const drop_zone = html.querySelector(".editor-panel-structure .editor-panel-body");
		if (drop_zone) {
			drop_zone.addEventListener("dragover", (ev) => {
				ev.preventDefault();
			});
			drop_zone.addEventListener("drop", (ev) => {
				// If the drop target is a node, that node's own handler fires first
				// and stops propagation. If we get here, the drop missed all nodes
				// (empty area around/below the tree) — default to last child of root.
				ev.preventDefault();
	...
	_handle_drop(target_el, event, host) {
		...
		} else if (payload.source === "palette_fragment") {
			this._handle_fragment_drop(payload.fragment_id, target_id, pos, host, false);
		} else if (payload.source === "palette_builtin_fragment") {
			this._handle_fragment_drop(payload.fragment_id, target_id, pos, host, true);
	...
	_handle_tree_container_drop(event, host) {
		...
		// Default: append as last child of root (so GMs can quickly stack fragments)
		const root_id = this.working_bt.root._id;

		if (payload.source === "palette") {
			this._handle_palette_drop(payload.node_type, root_id, "inside", host);
		} else if (payload.source === "palette_fragment") {
			this._handle_fragment_drop(payload.fragment_id, root_id, "inside", host, false);
		} else if (payload.source === "palette_builtin_fragment") {
			this._handle_fragment_drop(payload.fragment_id, root_id, "inside", host, true);
		} else if (payload.source === "tree") {
			this._handle_tree_drop(payload.node_id, root_id, "inside", host);
		}
	}
	...
	async _handle_fragment_drop(fragment_id, target_id, position, host, is_builtin) {
		if (!this.working_bt || !fragment_id) return;

		// Resolve the fragment definition (user-created or built-in)
		let fragment;
		if (is_builtin) {
			fragment = get_builtin_fragment(fragment_id);
		} else {
			fragment = get_bt(fragment_id);
		}
		if (!fragment?.root) return;

		// Clone the fragment's tree with fresh node IDs
		const clone = clone_subtree(fragment.root, fragment.id);
		if (!clone) return;

		// Namespace variables so each inserted copy gets independent keys
		const prefix = generate_fragment_prefix();
		const fragment_name = fragment.name || fragment.id;
		const var_keys = new Set((fragment.variables || []).map((v) => v.key).filter(Boolean));
		prefix_fragment_variables(clone, prefix, var_keys);
		...
		// Resolve the target node and adjust position like palette drops
		const found = _find_node(this.working_bt.root, target_id);
		if (!found) return;

		let pos = position;
		if (pos === "inside" && !_can_accept_child(found.node)) pos = "after";
		if ((pos === "before" || pos === "after") && !found.parent?.children) pos = "inside";

		_insert_at_position(this.working_bt.root, target_id, pos, clone);
		this.selected_node_id = clone._id;
		await this._save_working_bt();
		ui.notifications.info(
			game.i18n.format("dc-npc-patrols.bt.fragment_inserted", { name: fragment_name })
		);
		host.render();
	}
	...
	_delete_node_by_id(node_id, host) {
		if (!this.working_bt || !node_id) return;
		if (this.working_bt.root._id === node_id) {
			ui.notifications.warn("Cannot delete the root node. Change its type instead, or wrap logic in a child node.");
			return;
		}
		const found = _find_node(this.working_bt.root, node_id);
		...
		if (this.selected_node_id === node_id) {

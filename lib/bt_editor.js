...
import { populate_light_condition_fields } from "./light_condition_node.js";
		...
		html.querySelectorAll("[data-node-prop]").forEach((el) => {
			el.addEventListener("change", () => {
				...
				if (found.node.type === "condition_character" && key === "check_type") {
					...
					host.render();
				} else if (found.node.type === "condition_light" && key === "mode") {
					host.render();
				} else if (found.node.type === "condition_character" && key === "equip_mode") {
					host.render();
				}
			});
		});

...
import { get_face_direction_option_list } from "./face_directions.js";
import {
...
export class BTEditorController {
	...
	_populate_direction_selects(html) {
		...
		const variable_opts = [];
		for (const def of this.working_bt?.variables || []) {
			if (def.type !== "direction_select") continue;
			const key = def.key?.trim();
			if (!key) continue;
			const tag = `{{${key}}}`;
			const label = def.label ? `${tag} (${def.label})` : tag;
		...
		const direction_opts = get_face_direction_option_list().map(
			(d) => `<option value="${escape(d.value)}">${escape(d.label)}</option>`
		);
		...
		html.querySelectorAll("select[data-node-prop='direction']").forEach((sel) => {
			...
			if (direction_opts.length) {
				inner += `<optgroup label="${escape(game.i18n.localize("dc-npc-patrols.editor.compass_directions"))}">${direction_opts.join("")}</optgroup>`;
			}
			sel.innerHTML = inner;
			sel.value = current_direction;
		});
	}

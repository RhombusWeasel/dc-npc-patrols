...
export async function prepare_behaviour_tab_context(actor) {
	...
	const combat_behavior = actor.getFlag(MODULE_ID, "combat_behavior") || "freeze";
...
export function wire_behaviour_tab_events(html, sheet) {
	...
	root.querySelector("[data-combat-behavior]")?.addEventListener("change", async (ev) => {
		await actor.update(
			{ flags: { [MODULE_ID]: { combat_behavior: ev.currentTarget.value } } },
			{ render: false }
		);
	});
...
export function wire_hub_bt_variable_events(html, actor, bt_id) {
	if (!html || !actor || !bt_id) return;
	const section = html.querySelector(".hub-bt-variables");
	if (section) wire_bt_variable_events(section, actor, bt_id);
}

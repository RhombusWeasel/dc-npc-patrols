...
import { get_trees } from "./dialog_tree_store.js";
...
export async function prepare_flags_tab_context(actor) {
	return {
		...
		seen_trees: _seen_trees_for(actor.getFlag(MODULE_ID, "seen_nodes")),
	};
}

/**
 * Conversation-state rows grouped per tree. seen_nodes shape is
 * { tree_id: { node_or_response_id: true } } — flattened to
 * [{ tree_id, tree_name, node_ids: [...] }] for the template.
 * Human tree names come from the dialog tree store when available.
 */
function _seen_trees_for(seen) {
	const trees = seen || {};
	const store = (() => {
		try { return get_trees(); } catch { return {}; }
	})();
	return Object.entries(trees).map(([tree_id, nodes]) => ({
		tree_id,
		tree_name: store[tree_id]?.name || tree_id,
		count: Object.keys(trees[tree_id] || {}).length,
	}));
}

/** Clear a tree's seen_nodes entry (or all trees when tree_id is null). */
async function _clear_seen(actor, tree_id) {
	const seen = foundry.utils.duplicate(actor.getFlag(MODULE_ID, "seen_nodes") || {});
	if (tree_id === null) {
...
async function _write_flags(actor, scope, rows) {
	...
	// Posse scope: the runtime "posse flag" is a per-actor copy broadcast at
	// write time (see dialog_runner _set_posse_flags). A GM editing one member's
	// posse section must therefore broadcast the same edit to every player
	// member of the actor's posse, or the other members keep stale copies.
	if (scope === "posse_quest_flags") {
		const posse = game.dc.posse?.get_posse_for_actor(actor);
		if (posse) {
			const members = game.dc.posse.get_player_members(posse.id).filter((m) => m.id !== actor.id);
			for (const member of members) {
				await _write_local_flags(member, scope, obj);
			}
		}
	}
	await _write_local_flags(actor, scope, obj);
}

/** Write a scope's flags to a single actor (no broadcast). */
async function _write_local_flags(actor, scope, obj) {
...
export function wire_flags_tab_events(html, sheet) {
	...
	// Conversation-state clear buttons (per tree / all trees).
	root.querySelectorAll("[data-action='clear-seen']").forEach((btn) => {
		if (btn.dataset.wired) return;
		btn.dataset.wired = "1";
		btn.addEventListener("click", async () => {
			const tree_id = btn.dataset.treeId ? btn.dataset.treeId : null;
			await _clear_seen(actor, tree_id);
			sheet?.render?.();
		});
	});
}

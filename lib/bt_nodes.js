 ...
 *
 * 22 node types across 4 categories:
 *   3 Composites: sequence, selector, parallel
 *   2 Decorators: inverter, cooldown
 *   7 Conditions: flag, time, combat, weather, day, location, in_region
 *  10 Actions:   patrol, move_to, move_to_region, flee, sleep, wake,
...
register_node("sequence", {
	...
	description: "Runs children in order. Fails if any child fails.",
	tick: async (node, bb, engine) => {
		for (const child of node.children || []) {
			const status = await engine._tick_node(child, bb);
...
register_node("selector", {
	...
	description: "Tries children in order. Succeeds on first success.",
	tick: async (node, bb, engine) => {
		for (const child of node.children || []) {
			const status = await engine._tick_node(child, bb);
...
// PARALLEL: run all children. Succeeds when N of M succeed.
// Fails if (M - N + 1) children fail (i.e. success is impossible).
register_node("parallel", {
	category: "composite",
	label: "Parallel",
	icon: "fa-solid fa-bars",
	description: "Runs children simultaneously. Succeeds when N succeed.",
	tick: async (node, bb, engine) => {
		...
		for (const child of children) {
			...
			if (status === Status.FAILURE) failures++;
		}
		if (successes >= required) return Status.SUCCESS;
		if (failures > children.length - required) return Status.FAILURE;
...
register_node("condition_time", {
	...
	tick: async (node, bb) => {
		const start = _parse_time(node.start_time);
		const end = _parse_time(node.end_time);
	...
	tick: async (node, bb, engine) => {
		...
		try {
			for (const wp of due) {
				...
				await engine.region_manager.update_for_waypoint(bb.token, bb.actor, wp);
...
register_node("action_move_to", {
	...
	tick: async (node, bb, engine) => {
		...
		// Compute or retrieve cached A* path
		const pf = engine.pathfinding;
		const path = pf.find_path(
			bb.scene,
			{ x: bb.token.x, y: bb.token.y, level_id: bb.level_id },
			{ x: dest.x, y: dest.y, level_id: dest.level_id ?? bb.level_id }
		);
		if (!path || !path.length) return Status.FAILURE;

		// Animate through path segments
		bb.moving = true;
		bb._currently_moving = true;
		try {
			for (const step of path) {
				// Level transition at stairs cell
				if (step.level_id !== bb.level_id) {
					const level = bb.scene.levels.get(step.level_id);
					await bb.token.update({
						level: step.level_id,
						elevation: level?.elevation?.base ?? bb.elevation,
					});
...
register_node("action_move_to_region", {
	...
	tick: async (node, bb, engine) => {
		...
		// Animate through path segments (same logic as action_move_to)
		bb.moving = true;
		bb._currently_moving = true;
		try {
			...
			// Fire arrival at the destination region — this triggers dcDialogTree
			// and other region behaviors attached to the destination region.
			...
			await engine.region_manager.update_for_waypoint(bb.token, bb.actor, {
				region_name: node.region_name,
			});
		} finally {
			bb.moving = false;
...
register_node("action_face_player", {
	...
	tick: async (node, bb) => {
		...
		let nearest = null, nearest_dist = Infinity;
		...
		const angle = Math.atan2(nearest.y - bb.token.y, nearest.x - bb.token.x);
		await bb.token.update({ rotation: angle });
		return Status.SUCCESS;
	},
	editor: {
		fields: [

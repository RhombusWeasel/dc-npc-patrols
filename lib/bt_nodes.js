 ...
 *                acquire_target, measure_range, fire_weapon, reload_weapon, modify_item, wander_region
 *
...
import { find_nearest_region, token_in_any_named_region } from "./region_utils.js";
...
// ACTION: FACE_TOKEN — rotate toward a blackboard target or nearest matching token
const FACE_TOKEN_EDITOR_FIELDS = [
...
async function _tick_face_token(node, bb) {
...
// Legacy type — same behaviour as action_face_target; defaults to nearest player.
...
// ACTION: CLOSE_ON_TARGET — path toward a blackboard target within range
...
// ACTION: RELOAD_WEAPON — reload equipped weapon via Deadlands ammo rules
...
// ACTION: END_TURN — mark this NPC's combat turn complete
...
register_node("action_wander_region", {
	...
	tick: async (node, bb, engine) => {
		...
		// dest.x/y are nav-cell coords; convert to pixels using cell_size
		// (grid.size / nav_resolution), NOT the full grid size.
		const grid_data = engine.pathfinding.get_grid_data(bb.scene);

...
export function get_token_center_px(token_doc) {
	return get_token_center_point(token_doc, token_doc?.parent ?? null);
}
...
export async function tick_close_on_target(bb, engine, node, move_key) {
	...
	const path = engine.pathfinding.find_path(bb.scene, source, goal, { exclude_token_id: bb.token.id, key_uuids: get_actor_key_uuids(bb.actor) });
	if (path === engine.pathfinding.PENDING_PATH) return RUNNING;
	if (!path?.length) {
	...
	const target_level = target_doc.level ?? target_doc._source?.level ?? null;
	bb[move_key] = create_move_state(path, bb, {

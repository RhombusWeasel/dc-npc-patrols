...
function _apply_local(ns, posse_id, bag) {
	...
	bag = bag || {};
	// Re-render subscribers (e.g. open-sheet refresh on quests change) may call
	// request_posse_state during their render — on the GM client that
	// broadcasts immediately and re-enters here. Skip notify when the bag is
	// unchanged, otherwise each redundant request spins the loop forever.
	const prev = by_posse.get(posse_id);
	if (prev && JSON.stringify(prev) === JSON.stringify(bag)) return;

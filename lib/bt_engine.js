...
export class BTEngine {
	...
	_update_blackboard(bb, token_doc, actor, scene, unixtime) {
		...
		const tree = get_bt(actor.getFlag(this.module_id, "bt_id"));
		const var_defs = tree?.variables || [];
		const actor_vars = actor.getFlag(this.module_id, "bt_variables") || {};
		const resolved = {};
		for (const def of var_defs) {
			const key = def.key;
			if (!key) continue;
			const val = actor_vars[key];
			resolved[key] = (val !== undefined && val !== '') ? val : (def.default ?? '');
		}
		bb.variables = resolved;
	...
	async _tick_node(node, bb) {
		...
		// We resolve onto a shallow copy so the original stored tree is never
		// mutated. Structural keys (_id, type, children, child) are preserved
		// from the original — children are resolved recursively when
		// _tick_node is called on them.
		let tick_node = node;
		if (bb.variables && Object.keys(bb.variables).length) {
			tick_node = { ...node };
			for (const [k, v] of Object.entries(node)) {
				if (k === '_id' || k === 'type' || k === 'children' || k === 'child') continue;
				tick_node[k] = _resolve_value(v, bb.variables);
			}
		}

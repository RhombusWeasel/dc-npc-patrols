...
export class BTEngine {
	...
	// Called on time poll (alongside legacy engine for NPCs without BTs)
	async tick() {
	...
	_create_blackboard(token_doc, actor, scene) {
		return {
			...
			hidden: false,
			level_id: token_doc._source.level ?? scene.levels.contents[0]?.id ?? null,
	...
	_update_blackboard(bb, token_doc, actor, scene, unixtime) {
		...
		bb.level_id = token_doc._source.level ?? scene.levels.contents[0]?.id ?? null;

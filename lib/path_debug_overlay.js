...
export class PathDebugOverlay {
	...
	_render_struct() {
		...
		// ── 5. Terrain cost cells (yellow/orange tint) — current level only ──
		const terrain_costs = grid_data.terrain_costs;
		if (terrain_costs && terrain_costs.size > 0) {
			const level_costs = terrain_costs.get(current_level);
			if (level_costs && level_costs.size > 0) {
				for (const [key, cost] of level_costs) {
					const [tx, ty] = key.split(',').map(Number);
					// Higher cost = more orange, lower cost = more yellow
					const intensity = Math.min((cost - 1) / 4, 1); // clamp at cost=5
					const r = Math.round(255 * (0.8 + 0.2 * intensity));
					const g = Math.round(255 * (0.8 - 0.3 * intensity));
					const b = Math.round(255 * (0.2 - 0.2 * intensity));
					const color = (r << 16) | (g << 8) | b;
					gfx.lineStyle(1, color, 0.6);
					gfx.beginFill(color, 0.25);
					gfx.drawRect(tx * cell_size + 1, ty * cell_size + 1, cell_size - 2, cell_size - 2);
					gfx.endFill();
				}
			}
		}

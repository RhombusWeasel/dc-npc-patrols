...
export function migrate_node_types(node) {
	...
	// Canonicalize `lines` to the `;`-separated string form on chat nodes.
	// Older editor saves stored it as an array (split on `;`); chat.js now
	// handles both, but the canonical stored shape is a string.
	if (node.type === "action_chat" && Array.isArray(node.lines)) {
		node.lines = node.lines.join(";");
	}

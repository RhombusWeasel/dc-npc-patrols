...
class ConversationPanel extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	async _handle_response(response_id) {
		...
		if (response.set_flags) {
			...
			// Values and keys resolve against dialog variables + prose
			// placeholders, so a shared tree/fragment can write
			// per-NPC flag content (e.g. { drift_topic: "{{revealed_by}}" }).
			const resolved_flags = _resolve_flag_values(response.set_flags, this._variables, this.npc_actor, this.player_actor);
			if (scope === "posse") {
...
function _resolve_flag_values(flags, variables, npc_actor, player_actor) {
	...
	for (const [key, value] of Object.entries(flags)) {
		const k = typeof key === "string" && variables && Object.keys(variables).length
			...
			: key;
		out[k] = typeof value === "string" ? _resolve_text(value, variables, npc_actor, player_actor) : value;
	}
	return out;
}

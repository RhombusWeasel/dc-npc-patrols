...
class ConversationPanel extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	_resolve_node(node_id) {
		...
		while (current && this.nodes?.[current] && !visited.has(current)) {
			...
			if (_check_flag_conditions(node.flag_conditions, this.player_actor, this.npc_actor)) {
				// Conditions matched — divert to the target node and resolve it in
				// turn (supports divert chains, e.g. tri-state welcome states).
				const divert = node.flag_conditions_else_goto;
				if (divert && this.nodes?.[divert]) {
					current = divert;
					continue;
				}
				return current;

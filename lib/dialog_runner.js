 ...
 *     (uninformed actor default). Expansion repeats until stable (bounded),
 *     so a variable's value may itself carry {{other_var}} tokens.
...
function _set_flags_from_text(text, variables, npc_actor, player_actor) {
	...
	// Expand iteratively so a variable's VALUE may itself contain {{other_var}}
	// tokens (composed flag strings via dialog_variables). Bounded: unknown
	// keys stay literal, and a value that re-expands to itself cannot loop.
	let expanded = raw;
	for (let pass = 0; pass < 5; pass++) {
		const next = expanded.replace(/\{\{(\w+)\}\}/g, (match, name) =>

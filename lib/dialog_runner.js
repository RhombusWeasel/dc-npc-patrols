...
class ConversationPanel extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	async _prepareContext(_options) {
		...
		const visible_responses = responses.filter((r) =>
			_check_flag_conditions(r.flag_conditions, this.player_actor, this.npc_actor),
		);
	...
	async _handle_response(response_id) {
		...
		if (response.set_flags) {
			...
			if (scope === "posse") {
				await _set_posse_flags(this.player_actor, response.set_flags, this.npc_actor);
			} else {
		...
		while (current && this.nodes?.[current] && !visited.has(current)) {
			...
			for (const d of node.diverts || []) {
				if (d?.goto && this.nodes?.[d.goto] &&
					_check_flag_conditions(d.conditions, this.player_actor, this.npc_actor)) {
					divert = d.goto;
					break;
				}
			}
			// Legacy single condition + divert pair (pre-diverts trees).
			if (!divert && node.flag_conditions_else_goto && node.flag_conditions?.length &&
				_check_flag_conditions(node.flag_conditions, this.player_actor, this.npc_actor) &&
				this.nodes?.[node.flag_conditions_else_goto]) {
				divert = node.flag_conditions_else_goto;
			}
			if (divert) {
				current = divert;
				continue;
 ...
 * @returns {boolean}
 */
function _check_flag_conditions(conditions, actor, npc_actor = null) {

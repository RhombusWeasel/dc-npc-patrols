...
class ConversationPanel extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	async _prepareContext(_options) {
		...
		// Hide "knowledge-sharing" responses when this actor would grant
		// nothing: a response whose set_flags are ALL {{variable}} values
		// resolving to "" (uninformed actor default) is not shown. Responses
		// with any literal flag value or no set_flags at all are unaffected.
		visible_responses = visible_responses.filter((r) => {
			if (!r.set_flags || !Object.keys(r.set_flags).length) return true;
			const { any } = _resolve_flag_values(r.set_flags, this._variables, this.npc_actor, this.player_actor);
			return any;
		});
	...
	async _handle_response(response_id) {
		...
		if (response.set_flags) {
			...
			// Values and keys resolve against dialog variables; entries whose
			// resolved value is "" are dropped entirely (an uninformed actor
			// writes nothing rather than clobbering a set flag with "").
			const { flags: resolved_flags } = _resolve_flag_values(response.set_flags, this._variables, this.npc_actor, this.player_actor);
			if (scope === "posse") {
...
/**
 * Resolve a set_flags map: `{{dialog_variable}}` placeholders in BOTH keys
 * and values resolve against the conversation's dialog variables, so a
 * shared fragment can write per-NPC flag content. Values additionally get
 * prose placeholder substitution ({npc_name} etc.); KEYS do NOT — prose
 * placeholders in keys are slugified by _interpolate_flag_key downstream,
 * and resolving them here (unslugified) would break that pipeline.
 * Non-string values (booleans, numbers) pass through untouched.
 *
 * An entry whose resolved value is "" (e.g. the variable's default on an
 * actor with no override) is DROPPED — an uninformed actor must not write
 * an empty flag over one an informed actor legitimately set. An entry
 * whose resolved KEY is empty is likewise dropped.
 * @param {Object} flags — { key: value }
 * @param {Object} variables — resolved dialog variables (this._variables)
 * @param {Actor} npc_actor
 * @param {Actor} player_actor
 * @returns {{ flags: Object, any: boolean }} resolved map + whether any entry survived
 */
function _resolve_flag_values(flags, variables, npc_actor, player_actor) {
	const out = {};
	for (const [key, value] of Object.entries(flags)) {
		const k = typeof key === "string" && variables && Object.keys(variables).length
			? key.replace(/\{\{(\w+)\}\}/g, (match, name) =>
				Object.prototype.hasOwnProperty.call(variables, name) ? String(variables[name] ?? "") : match)
			: key;
		if (k === "") continue;
		const v = typeof value === "string" ? _resolve_text(value, variables, npc_actor, player_actor) : value;
		if (v === "") continue;
		// A resolved key may carry a comma-separated flag list (the dialog
		// variable behind a {{placeholder}} holds "a=1, b" &c.) — each token
		// becomes its own flag: "name=value" sets that value, a bare token
		// inherits the response's literal value. Values (prose) are NEVER
		// split, so flavor text containing commas passes through untouched.
		for (const part of k.split(",").map((s) => s.trim()).filter(Boolean)) {
			const eq = part.indexOf("=");
			if (eq > 0) {
				const name = part.slice(0, eq).trim();
				const pv = part.slice(eq + 1).trim();
				if (name && pv !== "") out[name] = pv;
			} else {
				out[part] = v;
			}
		}
	}
	return { flags: out, any: Object.keys(out).length > 0 };
}

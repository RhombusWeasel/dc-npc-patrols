...
import {
	can_observer_see_token_doc,
	create_observer_vision_source,
...
export function build_token_record(observer_doc, target_doc, grid_size) {
	...
	return {
		...
		x: center.x,
		y: center.y,
		level_id: target_doc.level ?? target_doc._source?.level ?? null,
	};
}
...
export function resolve_actor_ref(bb, key = "target") {
	...
	if (token_doc?.actor) return resolve_actor(token_doc.actor, token_doc);
	return null;
}
...
export function matches_token_filter(token_doc, options = {}) {
	...
	if (disposition === "enemy") {
		if (token_doc.disposition === DISPOSITION_VALUES.friendly) return false;
	} else if (disposition !== "any") {
...
function _record_matches_filter(record, options = {}) {
	...
	const disposition = options.disposition || "any";
	if (disposition === "enemy") {
		if (record.disposition === DISPOSITION_VALUES.friendly) return false;
	} else if (disposition !== "any") {
		const expected = DISPOSITION_VALUES[disposition];
...
/**
 * Resolve which token document to face for BT face actions.
 * @param {object} bb
 * @param {object} node
 * @returns {TokenDocument|null}
 */
export function resolve_face_token_doc(bb, node) {
	if (!bb?.token || !bb?.scene) return null;

	const source = node.source || "blackboard";
	const filter_opts = _face_filter_options(node);

	if (source === "blackboard") {
		const target_key = (node.target_key || "target").trim() || "target";
		return resolve_token_ref(bb, target_key);
	}

	if (source === "blackboard_list") {
		const list_key = (node.blackboard_key || "visible_tokens").trim() || "visible_tokens";
		const record = find_closest_from_records(bb[list_key], bb.token, filter_opts);
		if (!record?.token_id) return null;
		return bb.scene.tokens.get(record.token_id) ?? null;
	}

	const record = find_closest_token(bb.token, filter_opts);
	if (!record?.token_id) return null;
	return bb.scene.tokens.get(record.token_id) ?? null;
}

/**
 * Normalize legacy face node fields onto the shared face schema.
 * Handles action_face_player (legacy) and action_face_target (legacy)
 * as well as the current action_face node.
 * @param {object} node
 * @returns {object}
 */
export function normalize_face_node(node) {
	if (node.type === "action_face_player" || node.use_visible_tokens != null) {
		return {
			...node,
			source: node.source || (node.use_visible_tokens ? "blackboard_list" : "scene_scan"),
			filter: node.filter || "players",
			max_range: node.max_range ?? node.range ?? 3,
		};
	}
	return {
		...node,
		source: node.source || "blackboard",
		max_range: node.max_range ?? node.range ?? 0,
	};
}
...
export function find_closest_from_records(records, observer_doc, options = {}) {
	...
	const prefer_same_level = options.prefer_same_level ?? true;
	const observer_level = observer_doc.level ?? observer_doc._source?.level ?? null;
	let best = null;
	let best_dist = Infinity;
	let best_same_level = null;
	let best_same_level_dist = Infinity;
	...
	for (const record of records) {
		...
		const record_to_store = build_token_record(observer_doc, target_doc, grid_size);

		if (dist < best_dist) {
			best_dist = dist;
			best = record_to_store;
		}

		if (prefer_same_level && observer_level != null && record_to_store.level_id === observer_level) {
			if (dist < best_same_level_dist) {
				best_same_level_dist = dist;
				best_same_level = record_to_store;
			}
		}
	}

	return best_same_level ?? best;
}
...
export function find_closest_token(observer_token_doc, options = {}) {
	...
	const max_range = Number(options.max_range) || 0;
	const prefer_same_level = options.prefer_same_level ?? true;
	const observer_level = observer_token_doc.level ?? observer_token_doc._source?.level ?? null;
	...
	try {
		for (const target_doc of scene.tokens) {
			...
			if (!can_observer_see_token_doc(observer_token_doc, target_doc, vision_source)) continue;

			const record_to_store = build_token_record(observer_token_doc, target_doc, grid_size);

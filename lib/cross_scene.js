import { TOKEN_MOVE_OPTS } from "./utils.js";
...
export class CrossScene {
	...
	async _find_or_create_token(actor, scene, waypoint) {
		...
		if (token_doc) {
			...
			await token_doc.move([wp], { ...TOKEN_MOVE_OPTS, animate: false });
			const upd = { hidden: false };
			if (waypoint.face_direction != null) {
				upd.rotation = waypoint.face_direction;
			}
			await token_doc.update(upd, { animate: false, diff: false });
		} else {
			const td_data = {
				actorId: actor.id,
				x: px_x,

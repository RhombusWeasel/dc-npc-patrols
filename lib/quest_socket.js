...
/**
 * Hook installed on `posse_api._on_ext_change` by register_quest_socket().
 * After the GM client persists a posse quest write (ext ns "quests"), the
 * system fires this with (posse_id, ns); broadcast fresh state to every
 * client (the GM's own local cache included) when the ns is ours.
 * @param {string} posse_id
 * @param {string} ns
 */
function posse_quest_change_broadcaster(posse_id, ns) {
	if (ns && ns !== "quests") return;
	_broadcast_state(posse_id);
}

/**
 * Full registration — call from dcReady in main.js:
 * socket listener + the posse ext-change hook that triggers broadcasts.
 */
function register_quest_socket() {
	register_socket();
	if (game.dc?.posse) {
		game.dc.posse._on_ext_change = posse_quest_change_broadcaster;
	} else {

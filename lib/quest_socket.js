...
/**
 * Read a posse's quests — authoritative store on the GM client, cache on
 * players.
 * @param {string} posse_id
 * @returns {Object} quests map
 */
function get_current_quests(posse_id) {
	return get_posse_state(posse_id, QUESTS_NS, { authoritative: true });
}
...
async function request_quest_write(op, { posse_id, quest_id, data } = {}) {
	const bag = { ...get_current_quests(posse_id) };
	if (op === "set") {
		bag[quest_id] = data;
	} else if (op === "delete") {
		delete bag[quest_id];

...
/**
 * Generate a unique fragment prefix for inserted fragment variable namespacing.
 * @returns {string} e.g. "f_a1b2c3d4"
 */
export function generate_fragment_prefix() {
	return `f_${foundry.utils.randomID()}`;
}

/**
 * Node types whose string fields hold quest flag keys, not BT variables.
 * These must NOT be prefixed during fragment variable namespacing.
 */
const NO_PREFIX_NODE_TYPES = new Set([
	"action_set_flag",
	"condition_flag",
]);

/**
 * Walk cloned fragment nodes and prefix all {{var_name}} placeholders and
 * condition_variable.variable_key fields with the given prefix.
 *
 * - {{var_name}} → {{f_<uuid>_var_name}} for each of the fragment's declared variable keys
 * - condition_variable.variable_key: is_enabled → f_<uuid>_is_enabled (no {{}} wrappers)
 * - set_flag / condition_flag nodes are skipped entirely (quest flag keys, not variables)
 *
 * @param {Object} node — cloned node subtree (mutated in place)
 * @param {string} prefix — the fragment prefix (e.g. "f_a1b2c3d4")
 * @param {Set<string>} var_keys — set of variable keys declared by the fragment
 */
export function prefix_fragment_variables(node, prefix, var_keys) {
	if (!node || !prefix || var_keys.size === 0) return;

	function _prefix_string(str) {
		if (typeof str !== "string" || !str.includes("{{")) return str;
		return str.replace(/\{\{(\w+)\}\}/g, (match, key) =>
			var_keys.has(key) ? `{{${prefix}_${key}}}` : match
		);
	}
	...
	function _walk(n) {
		...
		// Quest flag nodes — never touch their fields
		if (NO_PREFIX_NODE_TYPES.has(n.type)) {
			if (n.children) for (const c of n.children) _walk(c);
			if (n.child) _walk(n.child);
			return;
		}

		// Prefix {{var}} placeholders in all string fields (except structural keys)
		for (const [k, v] of Object.entries(n)) {
			if (k === "_id" || k === "type" || k === "children" || k === "child") continue;
			if (typeof v === "string") {
				n[k] = _prefix_string(v);
			} else if (Array.isArray(v)) {
				n[k] = v.map((item) =>
					typeof item === "string" ? _prefix_string(item) : item
			);
		}
		}

		// condition_variable.variable_key holds a literal key (no {{}}), so prefix directly
		if (n.type === "condition_variable" && n.variable_key) {
			const raw_key = n.variable_key.replace(/^\{\{(.+)\}\}$/, "$1").trim();
			if (var_keys.has(raw_key)) {
				n.variable_key = `${prefix}_${raw_key}`;
			}
		}
 ...
 *
 * When a prefix is provided (insert path), each fragment variable key is
 * prefixed and its label composed as "<Fragment Name>: <Original Label>"
 * so multiple inserts of the same fragment get independent namespaces.
 *
 * @param {Array} parent_vars
 * @param {Array} fragment_vars
 * @param {Object} [opts]
 * @param {string} [opts.prefix] — fragment prefix for variable keys
 * @param {string} [opts.fragment_name] — fragment display name for labels
 * @returns {{ merged: Array, conflicts: string[] }}
 */
export function merge_variables(parent_vars = [], fragment_vars = [], opts = {}) {
	const { prefix, fragment_name } = opts;
	...
	for (const frag_var of fragment_vars) {
		const original_key = frag_var?.key;
		if (!original_key) continue;

		const prefixed_key = prefix ? `${prefix}_${original_key}` : original_key;
		if (keys.has(prefixed_key)) {
			conflicts.push(prefixed_key);
			continue;
		}
		keys.add(prefixed_key);

		const new_var = foundry.utils.deepClone(frag_var);
		new_var.key = prefixed_key;
		if (prefix && fragment_name) {
			new_var.label = `${fragment_name}: ${frag_var.label || original_key}`;
		}
		merged.push(new_var);
	}

...
async function _on_dialog_token_enter(event) {
	...
	if (game.settings.get(MODULE_ID, "combat_freeze") && is_dc_combat_active()) return;
...
async function _on_ambient_token_enter(event) {
	...
	// Ambient memory: track heard lines per day, reset on new campaign day
	const today = _get_campaign_day();
	const mem_key = `ambient_memory`;
	let memory = player_actor.getFlag(MODULE_ID, mem_key) || {};
	let set_mem = memory[set_id] || { heard_lines: [], day: today };

	// Reset if day changed
	if (set_mem.day !== today) {
		set_mem = { heard_lines: [], day: today };
	}

	// Filter to unheard, non-blank lines. Blank lines would make
	// ChatBubbles.broadcast throw ("must provide a message string").
	const blank = i => !(set.lines[i] || "").trim();
	let available_indices = set.lines
		.map((_, i) => i)
		.filter(i => !set_mem.heard_lines.includes(i) && !blank(i));

	// If all lines heard (or all remaining are blank), skip this line
	if (!available_indices.length) {
		return;
	}

	// Pick random from unheard
	const pick = available_indices[Math.floor(Math.random() * available_indices.length)];
	const line = set.lines[pick];

	// Show as a token bubble instead of a chat message to keep the chat log clean
	if (canvas?.ready && canvas.hud?.bubbles && line) {
		const npc_token = canvas.tokens.placeables.find(
			t => t.document.actorId === actor_id || t.actor?.id === actor_id,
		);
		if (npc_token) {
			await canvas.hud.bubbles.broadcast(npc_token, line, { cssClasses: ["emote"] });
		}
	}

	// Update memory
	set_mem.heard_lines.push(pick);
	memory[set_id] = set_mem;
	await player_actor.setFlag(MODULE_ID, mem_key, memory);

	await player_actor.setFlag(MODULE_ID, cooldown_key, Date.now());
}

/**
 * Get the current campaign day as a string "YYYY-MM-DD".
 * Uses the DC system's campaign time (unixtime + longitude offset).
 */
function _get_campaign_day() {
...
export function register_dialog_behaviors() {
	...
	// Module-defined document types are namespaced: moduleId.typeName
	const dialog_type = `${MODULE_ID}.dcDialogTree`;
	const ambient_type = `${MODULE_ID}.dcAmbient`;

	CONFIG.RegionBehavior.typeLabels[dialog_type] = `TYPES.RegionBehavior.${MODULE_ID}.dcDialogTree`;
	CONFIG.RegionBehavior.typeLabels[ambient_type] = `TYPES.RegionBehavior.${MODULE_ID}.dcAmbient`;
	CONFIG.RegionBehavior.typeHints[dialog_type] = `TYPES.HINTS.RegionBehavior.${MODULE_ID}.dcDialogTree`;
	CONFIG.RegionBehavior.typeHints[ambient_type] = `TYPES.HINTS.RegionBehavior.${MODULE_ID}.dcAmbient`;

	CONFIG.RegionBehavior.dataModels[dialog_type] = DCDialogTreeBehaviorType;
	CONFIG.RegionBehavior.dataModels[ambient_type] = DCAmbientBehaviorType;
	CONFIG.RegionBehavior.typeIcons[dialog_type] = "fa-solid fa-comments";
	CONFIG.RegionBehavior.typeIcons[ambient_type] = "fa-solid fa-comment-dots";
}
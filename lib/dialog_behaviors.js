...
async function _on_dialog_token_enter(event) {
	...
	// Look up the tree
	const { get_tree } = await import("./dialog_tree_store.js");
	...
	const { find_dialog_attachment, is_in_time_window } = await import("./attachment_editor.js");
	...
	// Open conversation
	const { open_conversation } = await import("./dialog_runner.js");
...
async function _on_ambient_token_enter(event) {
	...
	// Look up the set
	const { get_ambient_set } = await import("./dialog_tree_store.js");
	...
	const { find_ambient_attachment, is_in_time_window } = await import("./attachment_editor.js");
...
export function register_dialog_behaviors() {
	CONFIG.RegionBehavior.typeModels.dcDialogTree = DCDialogTreeBehaviorType;
	CONFIG.RegionBehavior.typeModels.dcAmbient = DCAmbientBehaviorType;
}
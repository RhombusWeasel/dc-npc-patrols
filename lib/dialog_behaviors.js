...
async function _on_ambient_token_enter(event) {
	...
	// Filter to unheard lines
	let available_indices = set.lines.map((_, i) => i).filter(i => !set_mem.heard_lines.includes(i));

	// If all lines heard, reset (or skip if we want strict daily limit)
	if (!available_indices.length) {
		// All heard today — skip this line
		return;
	}
	...
	if (canvas?.ready && canvas.hud?.bubbles) {

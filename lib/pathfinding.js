...
export class Pathfinding {
	...
	pick_random_reachable_cell(scene, source, region_name, max_attempts = 8) {
		...
		return this.pick_random_reachable_cell_in_region(scene, source, region, max_attempts);
	}

	pick_random_reachable_cell_in_region(scene, source, region, max_attempts = 8) {
		if (!region) return null;
	...
	find_path_to_region(scene, source, region_name) {
		const region = find_nearest_region(scene, region_name, source);
		if (!region) return null;
		return this.find_path_to_region_doc(scene, source, region);
	}

	find_path_to_region_doc(scene, source, region) {
		const grid_data = this._get_or_build_grid(scene);
		...
		const cache_key = `${src_x},${src_y},${src_level}>region:${region.id}`;
		const scene_cache = this._path_cache.get(scene.id);
		if (scene_cache?.has(cache_key)) return scene_cache.get(cache_key);
	...
	find_path_to_wall(scene, source, wall) {
		const grid_data = this._get_or_build_grid(scene);
		...
		const cache_key = `${src_x},${src_y},${src_level}>wall:${wall.id}`;
		const scene_cache = this._path_cache.get(scene.id);
		if (scene_cache?.has(cache_key)) return scene_cache.get(cache_key);

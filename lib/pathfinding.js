...
function _reconstruct_path(came_from, end_key, grid_data) {
	...
	// Convert nav-cell coords to grid-tile coords, then collapse consecutive
	// steps that move in the same direction into single waypoints so the
	// token animates in straight line segments between turns.
	//
	// Each segment was verified wall-free by A* at nav-cell resolution, so
	// animating in a straight line between waypoints cannot cross walls.
	//
	// When nav_resolution > 1, multiple nav cells map to the same grid tile.
	// Steps that don't change the snapped grid tile (dir = 0,0) are skipped
	// for direction-tracking purposes — they must NOT reset last_dir, or
	// diagonal segments break into staircases.
	...
	for (let si = start_idx; si < raw_path.length; si++) {
		...
		if (prev_gx !== null) {
			...
			// No grid-tile movement — skip without updating direction tracking.
			// This prevents zero-movement steps from breaking diagonal segments.
			if (dir_x === 0 && dir_y === 0) {
				prev_gx = sx;
				prev_gy = sy;
				continue;
			...
			// If same direction as last actual movement, replace the last
			// waypoint (extend the straight line segment)

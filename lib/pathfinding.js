...
export class Pathfinding {
	...
	find_path(scene, source, dest, options = {}) {
		...
		// Run A*
		const path = this._a_star(grid_data, src_x, src_y, src_level, dst_x, dst_y, dst_level, blocked_by_level, scene.id, options.key_uuids);
	...
	find_path_to_region_doc(scene, source, region, options = {}) {
		...
		const path = this._a_star_multi_goal(grid_data, src_x, src_y, src_level, goal_cells, blocked_by_level, scene.id, options.key_uuids);
	...
	_a_star(grid_data, sx, sy, src_level, dx, dy, dst_level, blocked_by_level = null, scene_id = null, key_uuids = null) {
		...
		while (open.size > 0) {
			...
			const neighbors = this._get_neighbors(current.x, current.y, current.level, grid_data, blocked_by_level, scene_id, key_uuids);
	...
	_a_star_multi_goal(grid_data, sx, sy, src_level, goal_cells, blocked_by_level = null, scene_id = null, key_uuids = null) {
		const { grids, stairs_cells, gw, gh } = grid_data;
		const open = new MinHeap();
		const came_from = new Map();
		const g_score = new Map();

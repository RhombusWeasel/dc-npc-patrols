...
export class Pathfinding {
	...
	find_path(scene, source, dest, options = {}) {
		...
		// Check cache — include exclude_token_id in key since blocked cells differ per NPC
		const excl = options.exclude_token_id ?? 'none';
		const cache_key = `${src_x},${src_y},${src_level}>${dst_x},${dst_y},${dst_level}|${excl}`;
		const scene_cache = this._path_cache.get(scene.id);
		if (scene_cache?.has(cache_key)) return scene_cache.get(cache_key);

		// Run A*
		const path = this._a_star(grid_data, src_x, src_y, src_level, dst_x, dst_y, dst_level, blocked_by_level);
	...
	pick_random_reachable_cell_in_region(scene, source, region, max_attempts = 8, options = {}) {
		...
		// Compute token occupancy to skip occupied destination cells
		const blocked_by_level = this._get_token_cells_by_level(
			scene, cell_size, gw, gh, options.exclude_token_id,
		);
		const src_level = source.level_id ?? this._default_level(scene);
		const blocked_cells = blocked_by_level.get(src_level) ?? new Set();
		...
		for (let i = 0; i < attempts; i++) {
			...
			// Skip cells occupied by another token
			if (blocked_cells.has(`${cell.x},${cell.y}`)) continue;
	...
	find_path_to_region_doc(scene, source, region, options = {}) {
		...
		const excl = options.exclude_token_id ?? 'none';
		const cache_key = `${src_x},${src_y},${src_level}>region:${region.id}|${excl}`;
		const scene_cache = this._path_cache.get(scene.id);
		if (scene_cache?.has(cache_key)) return scene_cache.get(cache_key);

		const path = this._a_star_multi_goal(grid_data, src_x, src_y, src_level, goal_cells, blocked_by_level);
	...
	find_path_to_wall(scene, source, wall, options = {}) {
		...
		const excl = options.exclude_token_id ?? 'none';
		const cache_key = `${src_x},${src_y},${src_level}>wall:${wall.id}|${excl}`;
		const scene_cache = this._path_cache.get(scene.id);
		if (scene_cache?.has(cache_key)) return scene_cache.get(cache_key);
	...
	_process_walls_for_level(walls, level_id, gw, gh, cell_size, blocked_edges, door_data) {
		for (const wall of walls) {
			...
			if (wall.door > 0) {
				register_door(door_data, wall.id, level_id, gw, gh, wall.c, cell_size, is_closed_regular);
				if (!is_closed_regular) {
	...
	_get_or_build_grid(scene) {
		...
		const result = {
			grids, levels, stairs_cells, gw, gh, cell_size, nav_resolution: nav_res,
			door_edges: door_data.door_edges,
			door_sides: door_data.door_sides,
			door_by_wall: door_data.door_by_wall,
		};
	...
	_a_star(grid_data, sx, sy, src_level, dx, dy, dst_level, blocked_by_level = null) {
		...
		open.push({ x: sx, y: sy, level: src_level, f: _heuristic(sx, sy, dx, dy, src_level, dst_level) });
		...
		while (open.size > 0) {
			...
			const neighbors = this._get_neighbors(current.x, current.y, current.level, grid_data, blocked_by_level);
			for (const nb of neighbors) {
				...
				if (tentative_g < (g_score.get(nb_key) ?? Infinity)) {
					...
					const h = _heuristic(nb.x, nb.y, dx, dy, nb.level, dst_level);
	...
	_a_star_multi_goal(grid_data, sx, sy, src_level, goal_cells, blocked_by_level = null) {
		const { grids, stairs_cells, gw, gh } = grid_data;
		const open = new MinHeap();
		const came_from = new Map();
		const g_score = new Map();
	...
	_get_neighbors(x, y, level_id, grid_data, blocked_by_level = null) {
		const { grids, stairs_cells, gw, gh } = grid_data;
		const neighbors = [];
	...
	_get_token_cells_by_level(scene, cell_size, gw, gh, exclude_token_id = null) {
		...
		// Check if token blocking is enabled
		if (!game.settings.get(MODULE_ID, "block_tokens")) return result;

		for (const token of scene.tokens) {
			if (exclude_token_id && token.id === exclude_token_id) continue;

			// Skip hidden tokens — they shouldn't block pathfinding
			if (token.hidden) continue;

			// Determine the token's level
			const level_id = token._source.level ?? scene.levels.contents[0]?.id ?? '_default';
			if (!result.has(level_id)) result.set(level_id, new Set());
			const cells = result.get(level_id);

			// Token footprint in nav cells
			// Foundry tokens occupy width × height grid squares (in pixels: width*grid_size)
			...
			// Rasterize the token's footprint rectangle into nav cells

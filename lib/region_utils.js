...
export function token_in_any_named_region(token_doc, scene, name, cell_size = null) {
	...
	const center = get_token_center_point(token_doc, scene);
	const tx = Math.floor(center.x / grid);
	const ty = Math.floor(center.y / grid);

	// A token is only "in" a region when it is on the same level the region
	// occupies. Without this check, a token standing directly below a region
	// on a lower floor shares the same grid cell and is falsely reported as
	// inside it (e.g. Sheriff under his First Floor bed on Ground).
	const token_level = token_doc._source.level ?? scene.levels.contents[0]?.id ?? '_default';

	for (const region of find_regions_by_name(scene, name)) {
		// A region with no explicit levels applies to every level (matches the
		// pathfinder's `region.levels?.size ? [...region.levels] : all_levels`).
		if (region.levels?.size && !region.levels.has(token_level)) continue;

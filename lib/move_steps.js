...
import { find_doors_on_tile_step, open_door, close_door, find_keyed_doors_on_tile_step, set_door_state, get_actor_key_uuids } from "./doors.js";
...
export async function tick_move_path(bb, engine, move_key) {
	...
	try {
		...
		const current_level = step.level_id ?? bb.level_id;
		if (_tile_has_change_level(bb.scene, px, py, grid)) {
			let skip_to = index + 1;
			while (skip_to < path.length) {
				const s = path[skip_to];
				// Stop at the first step that is NOT on a changeLevel tile —
				// that's the first real walking step after exiting the stairs.
				if (!_tile_has_change_level(bb.scene, s.x * grid, s.y * grid, grid)) break;
				// Also stop if the level changed — that's the actual exit of
				// THIS stairway, not a different one further along the path.
				const s_level = s.level_id ?? bb.level_id;
				if (s_level !== current_level) break;
				skip_to++;
			}
			...
			if (!move_state.doors_opened) move_state.doors_opened = new Map();
			const key_uuids = get_actor_key_uuids(bb.actor);

			// Auto-open closed regular doors
			...
			for (const door of crossed) {
				...
				move_state.doors_opened.set(door.id, false); // false = don't re-lock
			}

			// Unlock + open keyed locked/secret doors
			const keyed = find_keyed_doors_on_tile_step(
				from_gx, from_gy, step.x, step.y, grid_data, bb.level_id, bb.scene, key_uuids
			);
			for (const door of keyed) {
				const was_locked = door.ds === CONST.WALL_DOOR_STATES.LOCKED;
				if (was_locked) await set_door_state(door, CONST.WALL_DOOR_STATES.CLOSED);
				await open_door(door);
				move_state.doors_opened.set(door.id, was_locked); // true = re-lock after
			}

			await engine.animate_to(bb.token, step);

			// Close + re-lock doors after crossing
			for (const door of crossed) {
			...
			for (const door of keyed) {
				if (move_state.doors_opened.has(door.id)) {
					await close_door(door);
					const should_relock = move_state.doors_opened.get(door.id);
					if (should_relock) await set_door_state(door, CONST.WALL_DOOR_STATES.LOCKED);
					move_state.doors_opened.delete(door.id);
				}
			}
...
async function _close_tracked_doors(move_state, scene) {
	...
	for (const [door_id, should_relock] of move_state.doors_opened) {
		const wall = scene.walls.get(door_id);
		if (!wall) continue;
		await close_door(wall);
		if (should_relock) await set_door_state(wall, CONST.WALL_DOOR_STATES.LOCKED);
	}

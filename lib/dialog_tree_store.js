...
/**
 * Migrate every dialog tree/fragment from the legacy
 * `flag_conditions` + `flag_conditions_else_goto` routing pair to the
 * ordered `diverts[]` array (first-match-wins). Idempotent: trees that
 * already carry diverts and no legacy pair are left untouched.
 *
 * Semantics of the faithful translation:
 *   legacy — IF node.flag_conditions match THEN jump to flag_conditions_else_goto
 *   diverts — one entry `{ conditions: <same conditions>, goto: <same target> }`
 *
 * Nodes whose legacy conditions were EMPTY with an else_goto (divert
 * unconditionally) are migrated the same way — a divert with an empty
 * conditions list always matches, which is the documented
 * _check_flag_conditions behavior (empty array = pass).
 *
	...
	for (const tree of Object.values(trees)) {
		...
		for (const node of Object.values(tree.nodes || {})) {
			...
			if (has_legacy && !node.diverts?.length) {
				node.diverts = [{
					conditions: node.flag_conditions || [],
					goto: node.flag_conditions_else_goto,
				}];

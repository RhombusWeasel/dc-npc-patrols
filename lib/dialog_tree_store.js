...
/**
 * Foundry requires every settings key to be registered before set/get.
 * Per-tree keys are dynamic (one per tree id), so register on demand.
 * register() is idempotent (re-registration with the same type is a no-op).
 */
function _ensure_tree_setting_registered(id) {
	const key = _tree_setting_key(id);
...
export async function migrate_dialog_tree_store() {
	...
	for (const tree of raw) {
		if (!tree?.id) continue;
		_ensure_tree_setting_registered(tree.id);
...
export function get_trees() {
	...
	for (const id of ids) {
		_ensure_tree_setting_registered(id);

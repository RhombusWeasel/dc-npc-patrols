 ...
 * A single key's wall_uuid may hold a comma-separated list of wall UUIDs
 * (e.g. "Scene.a.Wall.1, Scene.a.Wall.2") — each is added separately.
 * @param {Actor} actor
...
export function get_actor_key_uuids(actor) {
	...
	for (const key of Object.values(keys)) {
		if (!key?.wall_uuid) continue;
		for (const part of String(key.wall_uuid).split(",")) {

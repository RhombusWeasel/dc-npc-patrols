...
/**
 * Check whether the resolved weapon has no loaded rounds.
 * @returns {boolean}
 */
export function is_weapon_empty(actor, options = {}) {
	...
	const weapon = resolved.weapon ?? get_gear_item(actor, resolved.weapon_path);
	if (!weapon) return true;

	game.dc.ammo.migrate_weapon_ammo_fields(weapon);
	const loaded = weapon.loaded_rounds;

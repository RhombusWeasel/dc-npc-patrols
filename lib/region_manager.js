...
export class RegionManager {
	...
	/**
	 * Reposition a region to center it on the given token's current position.
	 * @param {Scene} scene
	 * @param {TokenDocument} token_doc
	 * @param {string} region_uuid
	 */
	async update_region_position(scene, token_doc, region_uuid) {
		...
		const center = token.center;
		const radius_px = region.shapes[0]?.radius ?? (2 * scene.grid.size);

		await region.update({
			shapes: [{ type: "circle", x: center.x, y: center.y, radius: radius_px }],
		});
	}
	...
	async _sync_one(scene, actor, attachment, kind) {
		...
		for (const token_doc of token_docs) {
			if (attachment.region_uuid) {
				// Region exists — reposition it
				await this.update_region_position(scene, token_doc, attachment.region_uuid);
			} else {
		...
		for (const region of scene.regions) {
			for (const behavior of region.behaviors) {
				if (behavior.system?.actor_id === actor_id &&
					(behavior.type === "dcDialogTree" || behavior.type === "dcAmbient")) {

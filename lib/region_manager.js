 ...
 *
 * Regions are only created when an actor has dialog/ambient attachments
 * configured. No empty waypoint-proximity regions are created.
 */
...
export class RegionManager {
	...
	/**
	 * Remove orphaned Phase 1 waypoint-proximity regions left on the scene
	 * from previous versions. These are named "<token> Proximity" and have
	 * no behaviors and no token attachment.
	 *
	 * Called on canvasReady to clean up legacy data.
	 * @param {Scene} scene
	 */
	async cleanup_orphaned_waypoint_regions(scene) {
		if (!scene) return;

		const to_delete = [];
		for (const region of scene.regions) {
			// Match the old naming pattern and absence of behaviors + attachment
			if (region.name?.endsWith(" Proximity")
				&& (!region.behaviors || region.behaviors.length === 0)
				&& !region.attachment?.token) {
				to_delete.push(region.id);
			}
		}
		if (to_delete.length) {
			console.log(`[dc-npc-patrols] Cleaning up ${to_delete.length} orphaned waypoint-proximity region(s)`);

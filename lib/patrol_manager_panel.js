...
export class PatrolManagerPanel extends HandlebarsApplicationMixin(ApplicationV2) {
	...
	// --- Track which actor is selected + active tab ---
	_selected_actor_id = null;
	_active_tab = "patrols"; // "patrols" | "attachments" | "behaviour"
	...
	async _prepareContext(options) {
		...
		// Pre-compute day states for each path
		...
		// Prepare attachment context for the attachments tab
		const attachment_ctx = await prepare_attachment_context(selected_actor);

		// Get available BTs and assigned BT
		const bts = get_bts();
	...
	async _onRender(context, options) {
		...
		// Tab switching
		html.querySelectorAll("[data-tab-select]").forEach((el) => {
			el.addEventListener("click", (ev) => {
				this._active_tab = ev.currentTarget.dataset.tabSelect;
				this.render();
			});
		});

		// Wire attachment editor events (Phase 3)
		...
		if (region_manager) {
			wire_attachment_events(html, selected_actor, region_manager);
		}
		...
		html.querySelector("[data-bt-assign]")?.addEventListener("change", async (ev) => {
			...
			ui.notifications.info(`BT assigned: ${bt_id || '(none)'}`);
			this.render();
		});

		// Open BT editor
		html.querySelector("[data-action='open-bt-editor']")?.addEventListener("click", () => {
			import("./bt_editor.js").then(({ BTEditor }) => {
				new BTEditor().render(true);
			});
		});

		// Toggle BT pause/resume
		...
		// Waypoint checkbox live-save (flee_target, home)
	...
	async _record_path(path_idx) {
		...
		// Hide panel so GM can interact with canvas (avoid minimize/maximize lifecycle issues)

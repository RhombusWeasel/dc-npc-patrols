...
// System condition evaluator — direct import used only when the deployed
// system has not attached `game.dc.condition_eval` yet.
import * as sys_condition_eval from "../../../systems/Deadlands-Classic/module/lib/condition_eval.js";
...
export function tick_character_condition(node, bb) {
	...
	const actual = _condition_eval().resolve_value(actor, node);
	if (actual === undefined) return Status.FAILURE;

	const expected = node.expected_value;
	return _condition_eval().evaluate(actual, operator, expected) ? Status.SUCCESS : Status.FAILURE;
}

/** Resolve the system condition evaluator, falling back to a direct import. */
function _condition_eval() {
	const ce = game.dc?.condition_eval;
	if (ce) return ce;
	return sys_condition_eval;
}

...
export function tick_character_condition(node, bb) {
	...
	const actual = game.dc.condition_eval.resolve_value(actor, node);
	if (actual === undefined) return Status.FAILURE;

	const expected = node.expected_value;
	return game.dc.condition_eval.evaluate(actual, operator, expected) ? Status.SUCCESS : Status.FAILURE;
}

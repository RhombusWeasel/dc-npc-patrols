 ...
 *
 * Upstream re-checks only re-tick condition nodes (leaf conditions).
 * Composites and decorators are NOT re-ticked — a completed composite has no
 * saved state, so re-ticking it would re-run its children from scratch,
 * including movement actions that create phantom paths back to previous
 * waypoints and fight the downstream movement.
 */
...
/**
 * Node types that are completely skipped by composites — not executed,
 * not counted, not part of the flow. Comment nodes are pure documentation.
 */
const SKIPPABLE_NODE_TYPES = new Set(["comment"]);

/**
 * Filter out comment (and other non-executable) nodes from a children array.
 * Composites call this before iterating so comment nodes are invisible
 * to the engine flow.
 * @param {Array} children
 * @returns {Array}
 */
export function filter_executable_children(children) {
	if (!children?.length) return [];
	return children.filter((c) => c && !SKIPPABLE_NODE_TYPES.has(c.type));
}
...
/**
 * Whether a node should be re-ticked during upstream reactive re-checks.
 * Only condition nodes (leaf conditions) are re-evaluated — they are stateless
 * and safe to re-tick.  Composites and decorators are skipped because a
 * completed composite/decorator has no saved state, and re-ticking it would
 * re-run its children from scratch, including movement actions that create
 * phantom paths back to previous waypoints.
 * @param {object|null} node
 * @returns {boolean}
 */
function _needs_recheck(node) {
	const category = NODE_REGISTRY[node?.type]?.category;
 ...
 * Action nodes are skipped — only conditions/composites/decorators are re-evaluated.
...
export async function recheck_upstream_sequence(children, resume, bb, engine) {
	...
	for (let i = 0; i < resume; i++) {
		if (!_needs_recheck(children[i])) continue;
...
export async function recheck_upstream_ordered(children, order, resume, bb, engine, mode) {
	...
	for (let j = 0; j < resume; j++) {
		if (!_needs_recheck(children[order[j]])) continue;

...
export default async function flag_condition(boon, context) {
	...
	// Awaited: nested modify_flag posse writes are async; completing them
	// before the caller navigates prevents the resolver race (2026-08-31).
	if (satisfied) {
		for (const nested of boon.satisfied_boons || []) {
			await game.dc.boon_manager.handleBoon(nested, context);
		}
	} else {

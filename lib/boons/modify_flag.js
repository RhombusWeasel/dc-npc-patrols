 ...
 *
 * Modes:
 *   'set'       — creates or updates a flag to an absolute value
 *   'increment' — adds flag_value (as a number) to the current flag value;
 *                 missing/non-numeric flag treated as 0
 ...
 *
 * Actor-scope sets/increments accumulate into context.pending_flag_sets
 * (flushed by resolve_context.js after all boons run).  Posse-scope and
 * delete operations are applied immediately since they need multi-actor or
 * unsetFlag handling.
 *
...
export default async function modify_flag(boon, context) {
	...
	if (mode === 'delete') {
		// Awaited so delete completes before subsequent boons / navigation.
		await _apply_delete(actor, scope_type, full_path);
		return;
	}

	if (mode === 'increment') {
		const delta = Number(boon.flag_value) || 0;
		if (scope_type === 'posse') {
			// Awaited: the roll-gate resolver can render before a fire-and-forget
			// write lands, filtering out every branch (player-side race, 2026-08-31).
			await _apply_posse_increment(actor, key, delta);
		} else {
			if (!context.pending_flag_sets) context.pending_flag_sets = [];
			context.pending_flag_sets.push({
				actor, scope: MODULE_ID, path: full_path,
				value: delta,
				op: 'increment',
			});
		}
	...
	if (scope_type === 'posse') {
		// Awaited — see increment branch note. Must complete before
		// resolve_context returns so the conversation re-render sees the flag.
		await _apply_posse_set(actor, key, value);
	} else {
...
/**
 * Increment a flag on all player members of the actor's posse.
 ...
 * @param {number} delta
 */
async function _apply_posse_increment(actor, key, delta) {
	const posse = game.dc.posse?.get_posse_for_actor(actor);
	if (!posse) {
		const current = Number(actor.getFlag(MODULE_ID, `quest_flags.${key}`)) || 0;
		await actor.setFlag(MODULE_ID, `quest_flags.${key}`, current + delta);
		return;
	}
	const members = game.dc.posse.get_player_members(posse.id);
	for (const member of members) {
		const current = Number(member.getFlag(MODULE_ID, `posse_quest_flags.${key}`)) || 0;
		await member.setFlag(MODULE_ID, `posse_quest_flags.${key}`, current + delta);
	}
}
